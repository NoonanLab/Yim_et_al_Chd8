...
sdata.batch <- readRDS('sdata_batch_SCT.rds') # output from 'SCTransform.R'
...
# compute anchors, CCA, with reference
batch.anchors <- FindIntegrationAnchors(object.list = sdata.batch, 
                                        reference = grep('F', names(sdata.batch)),
...
sdata.align <- RunUMAP(sdata.align, dims = dims.use, min.dist = 0.2)

# compute prelim clusters
sdata.align <- FindNeighbors(sdata.align, dims = dims.use) # default: k.param = 20, prune.SNN = 1/15
sdata.align <- FindClusters(sdata.align, resolution = seq(0.2, 0.6, by = 0.1))

# set Ident, resolution = 0.4
Idents(sdata.align) <- sdata.align$seurat_clusters <- sdata.align$integrated_snn_res.0.4

# saveRDS
saveRDS(sdata.align, 'sdata_align_RefF_prelim.rds')