...
# check, use cluster resolution = 0.4
# identical(Idents(sdata.align), sdata.align$integrated_snn_res.0.4)
# Idents(sdata.align) <- sdata.align$seurat_clusters <- sdata.align$integrated_snn_res.0.4

# compute cluster markers (~2 hr)
cluster.markers <- FindAllMarkers(sdata.align, assay = 'RNA', logfc.threshold = 0.5)
...
write_csv(cluster.markers.out, 'integrated_clusterMarkers_res0.4.csv')