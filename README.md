...
For each single-cell or single-nucleus sample, fastq reads were aligned to the mm10 reference genome using [STAR](https://doi.org/10.1093/bioinformatics/bts635/) v2.7.9a. Reads mapping to more than 10 loci were then discarded, as were multi-mapping reads with more than one best scoring alignment. Aligned reads passing these criteria were used to generate single-cell and single-nucleus count matrices using STARsolo. 
